...
(define-lustre-test expect-seq-not-equal-same-length
  (let ((lustre-tests:*max-diff-items-to-display* 3)
    ...
    (assert-failed-description
     ...
     "Levenshtein distance: 1, first diff at 0, showing from 0 to 0
Expected: a
Actual:   b
          ~")))
...
(define-lustre-test expect-seq-not-equal-first-shorter
  (let ((lustre-tests:*max-diff-items-to-display* 3)
    ...
    (assert-failed-description
     ...
     "Levenshtein distance: 1, first diff at 1, showing from 0 to 1
Expected: a  
Actual:   a b
            +")))
...
(define-lustre-test expect-seq-not-equal-same-length-too-long-to-show-start
  (let ((lustre-tests:*max-diff-items-to-display* 4)
    ...
    (assert-failed-description
     ...
     "Levenshtein distance: 1, first diff at 3, showing from 1 to 4
Expected: ...b c d e
Actual:   ...b c f e
                 ~  ")))
...
(define-lustre-test expect-seq-not-equal-same-length-too-long-to-show-end
  (let ((lustre-tests:*max-diff-items-to-display* 3)
    ...
    (assert-failed-description
     ...
     "Levenshtein distance: 1, first diff at 1, showing from 0 to 3
Expected: a b c d...
Actual:   a x c d...
            ~    ")))

(define-lustre-test expect-seq-not-equal-same-length-show-end-exactly
  (let ((lustre-tests:*max-diff-items-to-display* 4)
        (lustre-tests:*max-displayed-items-before-diff* 2))
    (assert-failed-description
     (lustre-tests:expect-seq "abcde" "axcde")
...
(define-lustre-test expect-seq-not-equal-same-length-too-long-to-show-start-end
  (let ((lustre-tests:*max-diff-items-to-display* 2)
        (lustre-tests:*max-displayed-items-before-diff* 2))
    (assert-failed-description
     (lustre-tests:expect-seq "abcdefg" "abcdxfg")
     "Levenshtein distance: 1, first diff at 4, showing from 2 to 5
Expected: ...c d e f...
Actual:   ...c d x f...
                 ~  ")))
...
(define-lustre-test expect-seq-not-equal-deletions-and-substitutions
  (let ((lustre-tests:*max-diff-items-to-display* 3)
    ...
    (assert-failed-description
     ...
     "Levenshtein distance: 2, first diff at 4, showing from 2 to 6
Expected: ...c d e f g...
Actual:   ...c d   f x
                 -   ~")))
  ...
  (let ((lustre-tests:*max-diff-items-to-display* 3)
    ...
    (flet ((conc (s1 char s2)
      ...
      (assert-failed-description
       ...
       "Levenshtein distance: 3, first diff at 5, showing from 2 to 7
    ...
    (macrolet ((print-colored (color text stream)
      ...
      (assert-failed-description
       ...
       (with-output-to-string (s)
         (format s "~A~%" "Levenshtein distance: 4, first diff at 4, showing from 0 to 9")
